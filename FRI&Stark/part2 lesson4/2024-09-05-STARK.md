...
note：$P_i(X)$的次数 $deg(P_i)<2^h$。
note：如果$\mathsf{D} \subset \mathbb{F}$，那么 $f_1, ..., f_w$的取值就在$\mathbb{F}$上。这与$\mathsf{D}$和AIR的trace domain $\mathsf{H}_0$ 互不相交并不冲突。
...
7.  **FRI协议**：$\mathcal{P, V}$共同利用FRI协议的辅助输入$\mathsf{aux}_{\mathsf{FRI}}$，执行FRI协议，验证 proximity to 函数 $g_{(\vec{r},q,answer, r^F)}:\mathsf{D} \to \mathbb{K}$ 的RS码 $\mathsf{RS}[\mathbb{K},\mathsf{D}, \rho]$。其中，$$g_{(\vec{r},\mathsf{q}, answer, r^F)}:= \sum_{(i,j) \in I} r^F_{(i,j)}\cdot \mathsf{Quotient}(f_i; \mathsf{q}\cdot g^j, \alpha_{i,j})(x) + \mathsf{Quotient}(f_k^{\vec{r}}; \mathsf{q}^a, \beta_k)(x).$$
...
1.  在FRI的Query阶段，$\mathcal{V}$ 调用FRI的验证者 $s$ 次。
